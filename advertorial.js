...
advertorial  = () => {
  ...
  const whatdoesitdo = ['What does it do?', 'How does it work?', 'Why is it great?', 'What exactly is minnow swarm?'];
  const youallow = ['You allow it', 'You authorize minnow swarm to', 'You allow it to', 'You let it'];
  ...
  const opensource = ['the auth mechanisms are safe, open source, extremely simple and easy to verify and you are free to leave at any time, if you ever need help our community is very active', 'minnow swarm uses ordinary '+vest+' authentication mechanisms, the trigger pages that calls the authentication mechanisms are opensourced and easy to inspect, you are free to leave at any time, if you have any problems we also have a very active community that have always provent to be helpful', 'we use standard authentication mechanisms provided by the blockchain, we use standard singing tools as provided by the blockchains documentation, you can leave at any time, the tools we use to join are easy to inspect, and we do not keep any of our users keys']
